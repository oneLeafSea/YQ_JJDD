...
@interface RTWSMgr() <SRWebSocketDelegate> {
    SRWebSocket *_ws;
    connectCpl _connCpl;
}
...
- (void)startTimer {
    self.timer = [NSTimer scheduledTimerWithTimeInterval:30.0f target:self selector:@selector(sendPing) userInfo:nil repeats:YES];
}
