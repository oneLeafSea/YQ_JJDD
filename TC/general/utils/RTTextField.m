...
- (void)layoutSubviews
{
    ...
    if (self.text.length > 0 && self.isFirstResponder) {
        //User entered some text in the textfield. Check if the delegate has implemented the required method of the protocol. Create a popover and show it around the RTTextField.
        ...
        if ([self.delegate respondsToSelector:@selector(dataForPopoverInTextField:)]) {
            ...
            NSLog(@"<RTTextField> WARNING: You have not implemented the requred methods of the RTTextField protocol.");
        }
    }
    else{
        ...
        if ([[self delegate] textFieldShouldSelect:self]) {
            if ([self applyFilterWithSearchQuery:self.text].count > 0) {
                ...
                if ([[self delegate] respondsToSelector:@selector(textField:didEndEditingWithSelection:)]) {
                    ...
                    NSLog(@"<RTTextField> WARNING: You have not implemented a method from RTTextFieldDelegate that is called back when the user selects a search suggestion.");
                }
            }
